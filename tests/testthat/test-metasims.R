context("metasims - this takes awhile")

default_metasims <- metasims(progress = TRUE, trials = 100)
...
# single_metasims <- metasims(single_study = TRUE, trial_fn = singletrial, progress = FALSE, trials = 100)
...
test_that("coverage is as expected", {
  ...
  expect_lt(default_metasims %>% pluck("coverage") %>% median(), 1.0001)
  expect_gt(default_metasims %>% pluck("coverage") %>% median(), 0.90)
  # expect_lt(single_metasims %>% pluck("coverage") %>% mean(), 1.0001)
  # expect_gt(single_metasims %>% pluck("coverage") %>% mean(), 0.85)
})
