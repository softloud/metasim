...
metasims_default <- metasims_test(progress = FALSE)
metasims_assymmetric_beta <-metasims_test(
  prop = 0.2,
  prop_error = 0.01,
  progress = FALSE
)
metasims_big <- metasims_test(
  min_n = 300,
  max_n = 350,
  progress = FALSE
)
...
test_that("metasims parses sample size arguments", {
  expect_lt(metasims_default %>% pluck("p") %>% mean(), 0.6)
  expect_gt(metasims_default %>% pluck("p") %>% mean(), 0.4)
  expect_lt(metasims_assymmetric_beta %>% pluck("p") %>% mean(),
    0.25
  )
  expect_gt(
    metasims_assymmetric_beta %>% pluck("p") %>% mean(),
    0.15
  )
  expect_gte(metasims_big %>% pluck("n") %>% min(),
  300)
  expect_lte(metasims_big %>% pluck("n") %>% max(),
  350)
})
